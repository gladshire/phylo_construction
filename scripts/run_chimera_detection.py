...
def run_blastx(transcripts, blast_db, threads, out_dir):
    ...
    cmd = ["blastx", "-db", blast_db, "-query", transcripts, "-evalue",
           "0.01", "-outfmt", "6", "qseqid", "qlen", "sseqid", "slen",
           "frames", "pident", "nident", "length", "mismatch", "gapopen",
           "qstart", "qend", "sstart", "send", "evalue", "bitscore", "-out",
           out_dir + blastx_output_file, "-num_reads", str(threads),
           "-max_target_seqs", "100"]
...
def run_chimera_detection(blastx_output_file, out_dir):
    ...
    cmd = ["python3", SCRIPTS_HOME + "detect_chimera_from_blastx_modified.py",
...
def remove_chimeras_from_fasta(transcripts, info_file, out_dir):
    ...
    chimeric_transcripts = transcript_base_name + ".chimera_transcripts.fa"

    transcripts_original = SeqIO.index(transcripts, "fasta")

    df = pandas.read_table(info_file, header = None)
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage:")
        print("python3 run_chimera_detection.py transcripts_fasta reference_proteome_fasta threads output_directory")
        sys.exit()
        ...
        path_transcript, file_transcript = os.path.split(transcripts)
        transcript_name = str(file_transcript)
        transcript_base_name = transcript_name.plit(".")[0]
        blastx_output_file = transcript_base_name + ".bastx"
        ...
        run_blastx(transcripts, out_dir + proteome_ref, threads, out_dir)
