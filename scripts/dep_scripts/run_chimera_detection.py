...
SCRIPTS_HOME = os.path.expanduser("~/miles/allele_phasing/phylo_construction/scripts/") # where all scripts are located
...
def run_chimera_detection(blastx_output_file,DIR):
	...
	if os.path.exists(DIR+cut_file) and os.path.exists(DIR+info_file):
		...
		cmd = ["python3",SCRIPTS_HOME+"detect_chimera_from_blastx_modifed.py",blastx_output_file,DIR]
		print (" ".join(cmd))
		os.system(" ".join(cmd))
